...
- [x] **Cross-form §4947(a) trust appearance** (2026-05-12) — verified empirically. Across the full harmonized output (4.0M 990 rows × 25 tax_years, 2.8M 990-EZ rows × 27 tax_years), **zero** rows carry `subsection_cd == 92`. Control: 990-PF has 58,321 such rows (5.40%, matching the ~6% prior). §4947(a) trusts file Form 990-PF exclusively; the 990 / 990-EZ schemas accommodate the code defensively but it never appears. Schema doc updated to replace the prior "in theory" hedge with the empirical finding.
