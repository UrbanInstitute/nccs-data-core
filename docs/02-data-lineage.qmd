...
The `nccsdata` bucket is publicly readable — analysts do **not** need AWS credentials. Pass `--no-sign-request` to the AWS CLI to list or download without configured creds:

```bash
# List partitions
aws s3 ls s3://nccsdata/processed_merged/core/ --no-sign-request

# Download one file
aws s3 cp s3://nccsdata/processed_merged/core/2020/990combined/core_2020_990combined.parquet . --no-sign-request

# Or fetch directly over HTTPS, no CLI required
curl -O https://nccsdata.s3.amazonaws.com/processed_merged/core/2020/990combined/core_2020_990combined.parquet
```

Anonymous list is scoped to the published tiers (`processed/`, `processed_legacy/`, `processed_merged/`). For DuckDB / R-arrow over S3, point at the parquet URIs directly — `open_dataset()` and glob queries work without credentials.
