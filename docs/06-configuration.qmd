...
| `ENABLE_PARQUET` | `FALSE` | Skip phase 9; no `.parquet` sibling files written next to the promoted `.csv` |
| `ENABLE_S3_UPLOAD` | `FALSE` | Skip all of phase 8's S3 syncs; promotion to `processed/` and parquet generation still happen if their flags are on |
| `ENABLE_UPLOAD_RAW` | `FALSE` | Don't sync `data/raw/soi_extracts/` to S3 |
| `ENABLE_UPLOAD_FORMS` | `TRUE` | Don't sync `data/raw/forms/` to S3 |
| `ENABLE_UPLOAD_INTERMEDIATE` | `TRUE` | Don't sync `data/intermediate/{unpacked,harmonized}/` to S3 |
...
CLI overrides on `R/run_pipeline.R` map to a subset of these (`--strict`, `--no-strict`, `--upload`, `--no-upload`, `--parquet`, `--no-parquet`, `--no-{download,unpack,harmonize,combined,quality,dictionary,render}`). `R/run_build_panel.R` additionally exposes `--no-merge`. Phase 7.5 (promote) has no on/off flag — it always runs when its orchestrator reaches it; data CSVs need to be in `data/processed*/` before phase 9 walks them. Sub-toggles for the upload tiers don't have CLI flags — edit `R/config.R` or override via env if you need them.
...
The fourth output series, `990combined`, is derived in phase 4 from the 990 + 990-EZ harmonized outputs on their 54 shared columns. It is not listed in `FORMS` because it's not separately downloaded / unpacked / harmonized.
