...
3. Default: `parallel::detectCores() - 1` (no upper cap; one core reserved for the parent + OS).

The env var takes precedence over the function arg so cron / CI can tune without code changes:

```bash
NCCS_RENDER_WORKERS=32 bash scripts/run_pipeline.sh --strict
```

Each worker peaks at ~500 MB – 1 GB during the Quarto/pandoc render, so on memory-constrained hosts throttle via the env var — e.g. on a 4 GB instance, `NCCS_RENDER_WORKERS=4` keeps headroom for the parent R session. On a c5.18xlarge (72 vCPU, 144 GB RAM), the uncapped default uses 71 workers and renders ~60 HTML reports in well under a minute.
